...
class PathLoader:
    ...
    def get_waypoints(self, profile_id: str) -> List[Dict[str, Any]]:
        """Restituisce la lista ordinata di waypoint (node_id + posizione) per un dato profilo."""
        ...
        return [
            {"node_id": nid, "pos": np.array(self.data['nodes'][nid]['pos'], dtype=float)}
            for nid in node_ids
        ]

    def get_node_metadata(self, node_id: str):
        """Recupera i metadati di un nodo specifico per la compliance e le specializzazioni di dominio."""
