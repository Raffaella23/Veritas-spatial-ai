...
export function giroDentro(THREE, renderer, radice, ambienti, opzioni = {}) {
  ...
  // Prima si decide DOVE guardare (costa raggi), poi si scatta (costa pixel).
  const daScattare = [];
  for (const z of ordinati) {
    const occhio = [z.centroidX, (z.y || 0) + altezzaOcchio, z.centroidZ];
    const nome = (z.nome || z.label != null ? 'ambiente ' + (z.nome || z.label) : 'un ambiente')
    ...
    if (misuraIsovista) {
      for (let i = 0; i < sonde; i++) {
        ...
        direzioni.push({ a, area });
      }
      direzioni.sort((p, q) => q.area - p.area);
      // ⚠️ Una direzione che vede meno di un metro quadro è il muro: si scarta,
      //    perché una fotografia di un muro occupa un turno dell'occhio (decine
      //    di secondi) e non porta nessuna testimonianza.
      direzioni = direzioni.filter((d) => d.area >= 1);
    } else {
      // Senza isovista non si indovina: quattro direzioni cardinali, e lo si dice.
      direzioni = [0, 1, 2, 3].map((k) => ({ a: k * Math.PI / 2, area: null }));
    }
    direzioni.slice(0, perAmbiente).forEach((d, k) => {
      daScattare.push({ occhio, a: d.a, area: d.area, nome, primo: k === 0 });
    });
  }
