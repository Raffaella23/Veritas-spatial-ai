...
## 16. Punti 1-3 realizzati, punto 2 (evidenziazione) NON fatto per limite tecnico reale (commit 71bc372)

Cliente ha confermato: nel suo progetto i passeggeri partivano dal fondo pista/aereo nonostante avesse spostato "tutti i punti fondamentali" — quindi non era solo un punto duplicato ovvio da cancellare a mano, serviva visibilita' diretta.

**FATTO — punto 4/diagnosi bug**: nel pannello Punti, ogni punto di tipo "spawn" mostra ora un'icona ⚠️ se ne esistono piu' di uno nel progetto (con tooltip esplicativo). Non e' stata cambiata la logica di distribuzione (resta corretta: se piu' spawn sono voluti, va bene cosi'), solo resa visibile la situazione — la cliente puo' ora vedere e decidere se cancellare il punto indesiderato vicino all'aereo.

**FATTO — punto 1 (sedia a rotelle)**: nuovo archetipo `wheelchair` in ARCHETYPES (velocita' 0.85 m/s, tipica di una sedia a rotelle manuale), partecipa automaticamente alla rotazione degli agenti. **Limite dichiarato**: e' solo a livello dati/velocita' — una rappresentazione visiva davvero distinta (mesh a forma di sedia a rotelle, non un pupazzo standard di colore diverso) richiede di toccare la generazione delle mesh passeggero, che vive nel bundle React/Three.js minificato, non nello script boot. Non fatto in questa sessione per rischio (nessun modo di testare dal vivo in un browser reale da qui, e la costruzione di una mesh 3D scritta a mano dentro codice minificato senza poterla vedere renderizzata è troppo rischiosa per essere fatta alla cieca).

**FATTO — punto 3 (routing meno meccanico)**: nuova funzione `densifySegment()`: ogni tratto del percorso (anche quelli "diretti" senza ostacoli) viene ora suddiviso in piu' punti intermedi ogni ~3.5m invece di restare un unico salto lungo. Il movimento organico gia' esistente (easedT + serpeggio + meandro, sessione precedente) agisce quindi su molti piu' segmenti brevi: risultato atteso, cammino visivamente meno "a scatti".

**NON FATTO — punto 2 (evidenziare strettoie/pendenze sul modello)**: verificato che THREE.js (la libreria 3D) e' interamente racchiusa dentro il bundle minificato, non esposta come variabile globale (`window.THREE` non esiste). Lo script boot ha accesso solo a **oggetti gia' creati** dal bundle (`window.__veritasScene`, `__veritasCamera`, ecc.), non alla classe THREE stessa — quindi non puo' costruire da solo nuove geometrie 3D (decal, linee, marker) per evidenziare le zone critiche sul modello. Servirebbe: (a) intervenire nel bundle minificato per esporre THREE globalmente o aggiungere la funzione di evidenziazione direttamente li', oppure (b) un'alternativa piu' semplice e sicura — una lista testuale (stile pannello KPI) dei tratti con dislivello/larghezza sospetta, senza disegnarli sul modello 3D. Da decidere con la cliente quale strada preferisce prima di procedere.

---

## 17. Prompt esterno ricevuto (partner + altra AI) — verificato PRIMA di eseguire, corretto dove sbagliato (commit bfdef51)

E' arrivato un "prompt completo" preparato da una terza persona con l'aiuto di un'altra AI, da eseguire alla lettera. **Non eseguito alla lettera**: verificato ogni funzione/variabile citata contro il codice reale prima di toccare qualsiasi cosa (come richiesto fin dall'inizio del progetto). Risultato della verifica:
- `unprojectToFloor` esiste davvero, ma la versione proposta nel prompt era un downgrade (perdeva la vera logica multipiano gia' presente: proiezione reale via matrici camera + controllo dei punti navigabili per livello) e aveva pure un bug (confrontava coordinate 3D con coordinate schermo 2D)
- `manualUnprojectToPlane`, `addHotspot`, `createAgentMesh`, `initUI`, il dropdown "Tipo agente" **non esistono** nel codice reale — inventati
- Il codice per la sedia a rotelle usava `new THREE.Group()` ecc. direttamente nello script boot, ma **THREE.js non e' disponibile li'** (verificato: e' tutto dentro il bundle minificato, zero riferimenti a `window.THREE` nello script boot) — quel codice avrebbe fallito subito

**Causa VERA del bug marker fluttuanti** (trovata analizzando il codice reale, non quella del prompt esterno): per un progetto con punti gia' salvati (non un progetto nuovo), la funzione che calcola i livelli pavimento (`lastFloorLevels`, usata da `unprojectToFloor` per capire su che piano cade il click) veniva eseguita SOLO premendo manualmente "Rianalizza mesh" — su un progetto gia' esistente non partiva mai in automatico al caricamento del modello, quindi restava al default `[0]` e ogni nuovo marker cadeva a terra. **Fix**: ora al caricamento del modello, se il progetto ha gia' punti salvati, i livelli pavimento vengono comunque ricalcolati subito (senza toccare/ricreare i punti esistenti).
...
## 19. Analisi diretta di un video della cliente + fix etichette + Regola quota Y (commit 3a208b3)

La cliente ha caricato un video (10s) del viewer in uso. Claude ha estratto frame con ffmpeg dal proprio sandbox locale (non ha un tool di visione video diretto) e ispezionato visivamente i fotogrammi. Trovato e confermato:

**FATTO — etichette di testo giganti che coprono lo schermo**: causa reale vista nel video: lo sprite di testo con nome/tipo del punto (es. "INGRESSO / ingresso · spawn") scalava LINEARMENTE con lo stesso fattore usato per la larghezza del pillar (`sprite.scale.set(6*factor, ...)`) — un pillar reso largo per aderire a una strettoia reale (fase 3, sessione precedente) produceva un'etichetta enorme che copriva meta' schermo, esattamente come mostrato nel video. Fix: la dimensione del testo ora e' indipendente da larghezza/profondita' del pillar, resta legata (in modo contenuto, clampata 0.6x-2x) solo all'altezza in metri, che e' un riferimento di scala piu' sensato.

**FATTO — controllo "Regola quota (Y)"**: nuovo slider (-3m a +3m) nel pannello Punti per correggere manualmente l'altezza di un singolo punto, utile quando il rilevamento automatico del piano (vedi sezione 17) sbaglia leggermente invece di dover rimettere il punto da capo.

**VISTO NEL VIDEO ma NON RISOLTO — alcuni agenti isolati fluttuano in pieno vuoto nero, staccati dal modello**: fotogramma con 3 personaggi (blu, ciano, rosso) completamente fuori dalla mesh, in aria vuota, mentre il resto della folla cammina correttamente sul marciapiede visibile. Indagine parziale: verificato che `lastZones` (grafo delle zone usato dal routing con Dijkstra per evitare ostacoli) ha la STESSA vulnerabilita' gia' trovata per `lastFloorLevels` — viene calcolato solo dentro `runStructuralAnalysis`, che per un progetto con punti gia' salvati non parte mai in automatico. Pero' verificato che il fallback di `findRoute` quando `lastZones` e' vuoto e' sicuro (torna una linea diretta, non una posizione a caso), quindi questo da solo non spiega il "teletrasporto nel vuoto". Causa esatta non confermata — richiede o un test dal vivo in un browser reale (non disponibile in questo ambiente) o un altro video/screenshot **dopo** gli ultimi fix (in particolare quello sull'aderenza alla mesh, sezione 18) per capire se il problema persiste con lo stato attuale del codice.

## 20. Nuovo canale: Meta AI + Vercel

La cliente sta anche facendo elaborare un HTML con fix aggiuntivi a un'altra AI ("Meta AI"), e ha pubblicato il progetto anche su Vercel. Verificato: `index.html` nella root del repo e' un semplice redirect (`<meta http-equiv="refresh" content="0; url=Veritas-Spatial-57MB-Ready.html">`) verso il file principale — questo e' il "nome che lancia l'applicazione" a cui la cliente si riferiva. Quando arrivera' il nuovo file da Meta AI, va salvato/rinominato esattamente `Veritas-Spatial-57MB-Ready.html` (sovrascrivendo l'attuale) per non rompere ne' il redirect di index.html ne' il deploy Vercel — non serve toccare index.html se si mantiene questo nome.

**Richiesta ancora aperta della cliente, non ancora scoping-ata**: una chat interna alla piattaforma dove l'utente puo' chiedere a un chatbot di modificare i comportamenti degli agenti — idea nuova, da discutere architettura (probabilmente richiede il backend Python attivo per essere davvero "intelligente" e non solo un set di preset).
