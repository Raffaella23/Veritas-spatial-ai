...
> ## 🔴 STOP — READ `PROJECT_CONSTRAINTS.md` FIRST
>
> This skill is vendored into the **VERITAS Spatial AI** repository and is **overridden** by
> [`PROJECT_CONSTRAINTS.md`](./PROJECT_CONSTRAINTS.md), located next to this file.
> Read it before applying **any** recommendation below. Where the two disagree, that file wins.
>
> The four traps that matter most, summarised:
>
> 1. **Never recommend a framework.** VERITAS is one self-contained HTML file — no React/Vue/Next,
>    **no Tailwind**, no shadcn/ui, no bundler, no `package.json`.
> 2. **Step 1 of the Workflow below will mislead you.** It falls back to `html-tailwind` when no
>    `package.json` is found. VERITAS has no `package.json`, so that fallback fires — and it is wrong here.
>    The real stack is **HTML + vanilla CSS custom properties + ES modules, inline**.
> 3. **`data/stacks/threejs.csv` is factually wrong for this project.** It assumes Three.js r128 UMD from
>    cdnjs. VERITAS runs **three@0.171.0 via an ESM importmap**. Its two "Critical" rules (pin to r128;
>    `CapsuleGeometry` does not exist) are both false here and must be ignored.
> 4. **Never touch `<script>` block 3** (the minified React/Three engine) or anything under
>    `Assets/core/` (the Python Core). Both are out of scope for UI work, permanently.
>
> Product direction is already decided in `design_brief.md` — this skill supplies palettes, type scales,
> motion and UX rules to *fill that brief in*, never to redirect it.
