...
const err=[],log=[]; p.on("pageerror",e=>err.push(e.message)); p.on("console",m=>log.push(m.text()));
...
const r = await p.evaluate(async () => {
  ...
  await prova("segnaletica letta", () => {
    const s = window.__veritasSegnaleticaTrovata;
    if (!s) return "mai eseguita";
    if (!s.famiglie.length) return "nessuna: " + (s.motivo||"?");
    return s.famiglie.map(f => Math.round(f.tinta)+"deg "+f.tipo+" dir="+Math.round(f.direzione)+" n="+f.punti);
  });
