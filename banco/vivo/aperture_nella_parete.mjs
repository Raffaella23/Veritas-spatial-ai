...
const FUORI = path.join(qui, "aperture_nella_parete.txt");
...
const X0 = Number(process.env.X0 || -12), X1 = Number(process.env.X1 || 3);
const r = await p.evaluate(([X0, X1]) => {
  const T = window.THREE, R = window.__veritasModelRoot, nm = window.__veritasNavmesh;
  const rc = new T.Raycaster(); const out = [];
  for (let z = -14; z <= 13; z += 0.5) {
    // il pavimento sotto, a meta' fascia
    rc.set(new T.Vector3((X0 + X1) / 2, 20, z), new T.Vector3(0, -1, 0)); rc.far = 40;
    const giu = rc.intersectObject(R, true)[0];
    const y = giu ? giu.point.y : 0.6;
    const riga = { z, pav: giu ? +giu.point.y.toFixed(2) : null, colpi: [] };
    for (const h of [0.3, 1.0, 1.6]) {
      rc.set(new T.Vector3(X0, y + h, z), new T.Vector3(1, 0, 0)); rc.far = X1 - X0;
      const c = rc.intersectObject(R, true).filter((i) => i.distance > 0.01);
      riga.colpi.push(c.length ? c.slice(0, 3).map((i) => (X0 + i.distance).toFixed(1) + ":" + (i.object.name || "?").slice(0, 14)).join(" ") : "libero");
    }
    // camminabile sui due lati e nel mezzo?
    const q = (x) => { try { const s = nm.sulCamminoCorrente([x, y, z], [0.3, 1, 0.3]); return s && s.ok ? "#" : "."; } catch (e) { return "?"; } };
    let st = ""; for (let x = X0; x <= X1; x += 0.5) st += q(x);
    riga.cammino = st; out.push(riga);
  }
  return out;
}, [X0, X1]);
scrivi("fascia x " + X0 + " → " + X1 + "  (cammino: # camminabile, . no, ogni 50 cm)");
for (const w of r) scrivi("z " + String(w.z).padStart(5) + " pav " + w.pav + "  " + w.cammino + "   petto: " + w.colpi[1] + "   | 30cm: " + w.colpi[0]);
