...
export function segmentZones(grid, ft, opts = {}) {
  ...
  //
  // ⚠️ 15/09/2026 (notte) — LA FORMA NON SI BUTTA PIÙ. Fino a ieri questo
  //    accumulatore teneva solo cells/sumX/sumZ (il centroide) e
  //    maxClearance: un accesso allungato e uno quadrato con la stessa area
  //    finivano nello STESSO oggetto {areaM2, centroidX, centroidZ}, e ogni
  //    disegno a valle (index.html ×4, veritas_cinema.js, veritas_montaggio)
  //    doveva indovinare un cerchio/quadrato dalla sola area — il "cubo" che
  //    sostituisce la zona invece di indicarla.
  //
  //    Le celle di ogni bacino sono già visitate qui, una volta sola: si
  //    aggiungono tre somme (sumXX/sumZZ/sumXZ, i momenti secondi grezzi)
  //    per ricavare — DOPO il giro, senza un secondo passaggio — la STESSA
  //    matematica degli assi principali già scritta in
  //    `assePrincipale()` (veritas_segnaletica.js): matrice di covarianza
  //    2x2, autovalori, angolo. Nessun algoritmo nuovo: lo stesso conto,
  //    fatto per somme incrementali invece che su un array di punti (qui i
  //    punti sono le celle della griglia, potenzialmente migliaia).
  ...
  for (let i = 0; i < w * h; i++) {
    ...
    let a = acc.get(L);
    if (!a) {
      a = { label: L, cells: 0, sumX: 0, sumZ: 0, sumXX: 0, sumZZ: 0, sumXZ: 0, maxClearance: 0 };
      acc.set(L, a);
    }
    const x = i % w, z = (i - x) / w;
    a.cells++; a.sumX += x; a.sumZ += z;
    a.sumXX += x * x; a.sumZZ += z * z; a.sumXZ += x * z;
    if (dist[i] > a.maxClearance) a.maxClearance = dist[i];
  }

  // Sotto questa soglia di celle il momento secondo è troppo rumoroso per
  // dire qualcosa di affidabile su orientamento/allungamento (una manciata
  // di celle non basta a distinguere "stretto e lungo" da "rumore di
  // campionamento"): si lascia null e resta il ripiego ad area esistente.
  const CELLE_MINIME_FORMA = 6;
  ...
  for (const a of acc.values()) {
    ...
    let formaLungo = null, formaLargo = null, formaAngolo = null;
    if (a.cells >= CELLE_MINIME_FORMA) {
      // Covarianza dai momenti grezzi: Var(X) = E[X²] - E[X]² — stesso
      // risultato di centrare prima, un solo giro invece di due.
      const mx = a.sumX / a.cells, mz = a.sumZ / a.cells;
      const sxx = a.sumXX / a.cells - mx * mx;
      const szz = a.sumZZ / a.cells - mz * mz;
      const sxz = a.sumXZ / a.cells - mx * mz;
      // Autovalori della 2x2 — identico a `assePrincipale()`.
      const tr = sxx + szz, det = sxx * szz - sxz * sxz;
      const rad = Math.sqrt(Math.max(0, tr * tr / 4 - det));
      const l1 = tr / 2 + rad, l2 = Math.max(0, tr / 2 - rad);   // l1 >= l2
      if (isFinite(l1) && l1 > 0) {
        let angolo;
        if (Math.abs(sxz) > 1e-9) angolo = Math.atan2(l1 - sxx, sxz);
        else angolo = sxx >= szz ? 0 : Math.PI / 2;
        // Rettangolo equivalente: per una distribuzione uniforme su [-L/2,L/2]
        // la varianza è L²/12, quindi L = 2*sqrt(3*varianza). È la stessa
        // ipotesi di densità uniforme con cui l'area viene già misurata
        // (celle libere contate una per una) — nessuna soglia nuova tarata
        // a mano.
        const lungoCelle = 2 * Math.sqrt(3 * l1);
        const largoCelle = 2 * Math.sqrt(3 * l2);
        const lungo = lungoCelle * cellSize, largo = largoCelle * cellSize;
        if (isFinite(lungo) && isFinite(largo) && lungo > 0 && largo > 0) {
          formaLungo = +lungo.toFixed(2);
          formaLargo = +largo.toFixed(2);
          formaAngolo = +angolo.toFixed(4);
        }
      }
    }
    ...
    zones.push({
      ...
      // 15/09/2026 — la forma vera, quando c'è abbastanza cella per fidarsene;
      // altrimenti null, e chi legge ripiega sull'area come faceva già.
      formaLungo, formaLargo, formaAngolo,
    });
  }
  ...
  /*
   * ⚠️ OGNI ANELLO LASCIA UN NUMERO, NON UNA RIGA DI LOG — deciso il 07/09/2026
   *    dopo che Raffaella ha chiesto: *«perche' abbiamo ancora tutti questi
   *    problemi dal momento che sono settimane che lavoriamo su questa parte?»*
   *
   *    La risposta misurata e' che la catena e' lunga e ogni anello si stacca
   *    IN SILENZIO: occhio -> testimonianza -> zone -> tappe -> cammino -> film.
   *    Quando uno si stacca, tutto il resto continua a girare e a produrre
   *    numeri plausibili. Tre volte in una settimana, sempre cosi'.
   *
   *    Una riga di console non basta: scorre via, e chi arriva dopo non ha modo
   *    di CHIEDERE se quel passo e' avvenuto. Quindi ogni passo lascia il suo
   *    conto in un posto interrogabile, e `veritas_catena.js` li mette in fila.
   */
  const taglio = {
    passaggiLarghiTenutiSeparati: tenutiSeparati,
    testimoniAriaAperta: testimoni.length,
    testimoniCalpestio: testimoniCalpestio.length,
    ambientiToltiPerMezzi: toltiPerMezzi,
    m2ToltiPerMezzi: +m2Tolti.toFixed(2),
  };
  if (typeof window !== "undefined") {
    try {
      window.__veritasTaglio = window.__veritasTaglio || {};
      window.__veritasTaglio[String(levelY)] = taglio;
    } catch (e) {}
  }

  return { labels, zones, gateways: live, taglio };
}
